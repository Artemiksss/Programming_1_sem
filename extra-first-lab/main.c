...
int cast(unsigned char *bytes) {
    return (int) ((bytes[3] << 16) + (bytes[2] << 8) + (bytes[1] << 4) + bytes[0]);
}

void gameLife(int **firstGen, fieldProperties bitmap) {
...
int main(int argc, char *argv[]) {
    ...
    for (int i = bitmap.height - 1; i >= 0; i--)
        for (int j = 0; j < bitmap.width; j++) {
            pixels[i][j].black = getc(input);
            pixels[i][j].white = getc(input);
        }
    ...
    for (int i = 0; i < bitmap.height; i++)
        for (int j = 0; j < bitmap.width; j++) {
            firstGen[i][j] = (pixels[i][j].black == 0 && pixels[i][j].white == 0)?1:0;
        }
    for (int i = 0; i < bitmap.height; i++)
        free(pixels[i]);
    free(pixels);
    ...
    for (int i = 1; i <= max_iter; i++) {
        ...
        int n = 0;
        unsigned char *newGen = (unsigned char *) malloc(bitmap.width * bitmap.height * 4);
